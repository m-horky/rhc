...
// GetConsumerUUID gets the consumer UUID from RHSM D-Bus service
...
// IsRegistered returns true, when system is registered
...
func RegisterWithUsernamePassword(username, password, organization string, environments []string, enableContent bool) ([]string, error) {
	var orgs []string
...
// RegisterWithActivationKey registers the system using activation keys
func RegisterWithActivationKey(orgID string, activationKeys []string, environments []string, enableContent bool) error {
...
// Unregister unregisters the system from RHSM
...
// Register tries to register system against Red Hat Subscription Management server (candlepin server)
func Register(ctx *cli.Context, enableContent bool, uiSettings UISettings) (string, error) {
	uuid, err := GetConsumerUUID()
	if err != nil {
	...
	if uuid == "" {
		...
		if uiSettings.IsRich {
			...
			s.Prefix = uiSettings.SmallIndent + "["
		...
		if len(activationKeys) > 0 {
			...
			var orgs []string
			if organization != "" {
				_, err = RegisterWithUsernamePassword(username, password, organization, contentTemplates, enableContent)
			} else {
				orgs, err = RegisterWithUsernamePassword(username, password, "", contentTemplates, enableContent)
...
// UISettings defines the UI configuration needed by RHSM operations
